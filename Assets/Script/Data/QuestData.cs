...
public class QuestData : MonoBehaviour
{
    ...
    {      
        ...
        */
        GameData.Instance._playerData._quest[_index]++;
        OnLevelSet();

        if(GameData.Instance._playerData._quest[_index] >= 10)
        {
            if(!GameData.Instance._playerData._quest.ContainsKey(_index+1))
            {
                GameData.Instance._playerData._quest.Add(_index+1, 0);
                
                if(GameData.Instance._questMN._quest.ContainsKey(_index+1)) 
                    GameData.Instance._questMN._quest[_index+1]._lock.SetActive(false);
            }
        } 
    }
    ...
    void Update()
    {
        if(_play_quest)
        {
            ...
            if(_time <= 0)
            {
                ...
                GameData.Instance._gm.GetGold(_reward_gold);
